...
import useStyles from "./DisplayAllColorsCss";
...
export default function DisplayAllColors(props){
    ...
    const handleOpen = (rowData) => {
        ...
        setColorName(rowData.colorname)
    ...
    const handleEdit = async() => {
        setOpen(false)
        var body = {colorid: colorId, colorname: colorName, categoryid: categoryId, subcategoryid: subcategoryId, productid: productId}
        var result = await postData('color/editcolordata', body)
        if(result.result)
        {
            Swal.fire({
                position: 'center',
                        ...
                        <div className={classes.root}>
                            <div className={classes.subdiv}>
                                <Grid container spacing={1}>
                                    ...
                                    <Grid item xs={12}>
                                        <CssTextField sx={{ input: { color: '#c6186d' } }} value={colorName} onChange={(event)=>setColorName(event.target.value)} label="Color Name" variant="outlined" fullWidth></CssTextField>
                                    </Grid>
                                    <Grid item xs={6}>
                                        <Button
