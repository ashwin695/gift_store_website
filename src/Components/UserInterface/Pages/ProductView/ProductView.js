...
import { Button, Divider, FormControl, Grid, InputLabel, MenuItem, Select, TextField, Snackbar } from "@mui/material";
...
export default function ProductView(){
    ...
    const handleBuyNow = (item) => {
        if( sizeData.length > 0 )
        {
            ...
            else
            {
                ...
                {
                    /* var body = {item, userWhatsapp, size, sizeId, color, price, offerprice, uploadedFiles, filesWishMsg, filesName, filesMsg} */
                    var body = { item, userWhatsapp, size, sizeId, color, price, offerprice, qty }
        ...
        else if( colorData.length > 0 )
        {
            ...
            else
            {
                ...
                {
                    /* var body = {item, userWhatsapp, size, sizeId, color, price, offerprice, uploadedFiles, filesWishMsg, filesName, filesMsg} */
                    var body = { item, userWhatsapp, size, sizeId, color, colorId, price1, offerprice1, qty }
                    dispatch({ type: 'Add_Products', payload: [item.productid, body]})
                    setRefresh(!refresh)
                    navigate(`/cart`)
                }  
    ...
    const handleAddToCart = (item) => {
        ...
        else
        {
            ...
            {
                var body = {item, userWhatsapp, size, sizeId, color, price}
                dispatch({ type: 'Add_Products', payload: [item.productid, body]})
                setRefresh(!refresh)
            }  
        }
                    ...
                    <div className={classes.root}>
                        ...
                        <Grid container spacing={2} className={classes.rootrow}>
                            ...
                            <Grid item xs={6} style={{ margin:'1.5% 0% 0% 1%' }}>
                                ...
                                <Grid item xs={12} className={classes.margin}>
                                    <Divider sx={{ margin:'1.5% 0% 1.5% 0%' }} />
                                    <div className={classes.descheading}>Once order is placed, our team will contact you for the customisation inputs.</div>
                                    <Divider sx={{ margin:'1.5% 0% 0.5% 0%' }} />
